...
| **Code Review แบบ Non-Interactive** | `claude ultrareview --json` | review สำหรับ CI; พิมพ์ผลออก stdout, exit 0 เมื่อสำเร็จ / 1 เมื่อ fail |
| **รันคำสั่ง Shell เบื้องหลัง** | `claude --bg --exec "<cmd>"` | รันคำสั่ง shell ใน session เบื้องหลังแบบแยกออกมา |
...
### Code Review แบบ Non-Interactive

```bash
# review โค้ดแบบ non-interactive สำหรับ CI/สคริปต์
# พิมพ์ผลออก stdout; --json = raw
# exit 0 เมื่อสำเร็จ / 1 เมื่อ fail
claude ultrareview --json
```

### รันคำสั่งเบื้องหลัง (Background Exec)

```bash
# รันคำสั่ง shell ใน session เบื้องหลังแบบแยกออกมา
