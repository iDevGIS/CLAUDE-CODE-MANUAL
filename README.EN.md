...
[![Claude Code](https://img.shields.io/badge/Claude_Code-v2.1.282-7B61FF?style=flat-square)](https://docs.claude.com/claude-code)
[![Manual](https://img.shields.io/badge/Manual-v1.75.0-22C55E?style=flat-square)](./CHANGELOG.md)
...
## 🆕 What's new — synced to Claude Code `v2.1.282`

> This manual tracks the latest Claude Code release — it already covers these 👇

| 🔥 New | Details |
|---|---|
| 🚀 **Opus 5.5 model** | `claude-opus-5-5` — the **new default Opus** (v2.1.280), 1M context, $4/$20 per Mtok with $0.20/Mtok cache reads (previously `claude-opus-5` since v2.1.219) |
| ✨ **Sonnet 5 model** | `claude-sonnet-5` — the **new default in Claude Code**, native 1M context, standard pricing $2/$10 per Mtok |
| 🧠 **Fable 5 model** | `claude-fable-5` — Mythos-class, **1M** context by default — the first Fable of the family (see Fable 5.1 below) |
...
| 🔐 **Sandbox credential mask** | `sandbox.credentials` `mode: "mask"` (v2.1.221) — sandboxed commands read a sentinel copy; the real value is substituted on egress |
| 🖥️ **Focus view (VS Code)** | hides tool activity behind an expandable per-turn summary — `Ctrl+Alt+F` (v2.1.221) |
| 🌿 **Background & `/fork`** | background sessions commit/push and open a draft PR only when needed · `/fork` gets its own worktree · `/status` shows `attached`/`unattended` (v2.1.221) |
| 🔒 **Remote Control auto-start** | user scope only, via `/config` — repo settings can turn it off but no longer on (v2.1.222) |
| 🛡️ **Auto mode screens `SendMessage`** | messages to other agent sessions go through the permission classifier before dispatch (v2.1.222) |
| 🗑️ **ultraplan removed** | the ultraplan feature (`/ultraplan`, "Refine with Ultraplan") is gone (v2.1.222) |
| 🔎 **`/review` = `/code-review`** | one command for the current diff or a PR (`/code-review <level> <pr#>`) · remembers your last effort level (v2.1.223) |
| 🧮 **Context-window enforcement** | `CLAUDE_CODE_DISABLE_1M_CONTEXT` holds every 1M model to 200K · auto-compact also bounds unknown model IDs (v2.1.223) |
| 🏬 **Marketplace wildcards** | `strictKnownMarketplaces` / `blockedMarketplaces` accept `"owner/*"` to cover a whole org in one line (v2.1.223) |
...
| 🔏 **claude.ai-synced skills sandboxed** | they can't shadow local commands or MCP prompts · their bodies don't run `!` or expand `@` on your machine (v2.1.228) |
| ✍️ **`Write` drops the read-first rule** | newer models can overwrite a file they haven't read, matching `Edit`'s rules (v2.1.228) |
| 🔌 **`command` marketplace source** | a local command (e.g. an IDE) prints the plugin directory · re-resolved each session with no restart · `mode: "link"` uses it in place (v2.1.229) |
| ↩️ **`claude remote-control --continue`** | resume the most recent Remote Control session (v2.1.229) |
| 🛑 **`/commit-push-pr` stops auto-approving risky flags** | `--force` · `--amend` · `--no-verify` and friends now need your approval (v2.1.229) |
| 🍴 **Subagent forking on by default** | `subagent_type: "fork"` inherits the full conversation and prompt cache · non-teammate agents run in the background by default (v2.1.232) |
| 💬 **Type `@` to mention another session** | mention a live session by name in the prompt and Claude reaches it with `SendMessage` · session names stay unique per machine (v2.1.232) |
| 🦊 **GitLab support** | marketplaces clone from bare `gitlab.com` URLs (nested subgroups included) · GitLab token families are redacted · `glab` gets `gh`-level protection (v2.1.232) |
...
| ✂️ **Built-in "Concise" output style** | a stock output style that leads with results and skips preamble and narration, while doing the work just as thoroughly — pick it under **Output style** in `/config` (v2.1.237) |
| ⌨️ **`keybindingFlavor` setting** | set it to `"readline"` and `Ctrl+W` deletes back to the previous whitespace, as in Bash — the `"classic"` default is unchanged · `Ctrl+L`/`Cmd+K` in fullscreen only repaint, the double-press `/clear` shortcut is gone (v2.1.238) |
| 🔑 **Marketplace `headersHelper`** | runs a command that mints HTTP headers (e.g. a short-lived token) for catalog and same-origin archive fetches · a catalog entry's helper runs only on install/update, after its command is shown — `claude plugin install/update` ask `[y/N]` (or pass `-y`) (v2.1.238) |
| 🖥️ **New self-hosted runner flags** | `--defer-shutdown-max-min` keeps serving attached sessions after `SIGTERM`, then parks the rest · `--proxy-authorization-command` / `--proxy-authorization-file` for proxies needing a fresh header per connection (v2.1.238) |
| 💰 **Cost estimates include the 1.1× premium** | `/cost` · status line · `--max-budget-usd` now include the 1.1× US-only-inference premium for data-residency workspaces (v2.1.239) |
...
| 🛡️ **Auto mode tab in `/permissions`** | view/edit auto mode classifier rules from the dialog · startup warning for wildcard-before-subcommand Bash allow rules like `Bash(git * main)` (v2.1.246) |
| 📂 **`/cd` applies immediately** | the new directory's settings, hooks, `.mcp.json`, skills and agents take effect right after the move — no `--resume` needed (v2.1.246) |
| 📝 **Claude drafts feedback reports** | when something goes wrong in a session, Claude drafts a report with the `SendFeedback` tool for you to review and send from `/feedback` · turn off with the `feedbackDrafts` setting (v2.1.247) |
| 💸 **`/claude-api cost-optimize`** | profiles a project's Claude API spend and works through caching · token hygiene · batch · effort · model choice one measured change at a time · the skill also gains Admin API coverage (v2.1.247) |
| 🗜️ **Sonnet 5 auto-compacts at full 1M** | its default auto-compact window now covers the whole 1M window — sessions on the 1M window compact at ~967K tokens instead of ~934K (v2.1.247) |
| 🔒 **`--restricted` flag** | removes the built-in tools that run commands or code and `WebFetch` (unless named in `--tools`) · keeps file tools inside the working directory · refuses `bypassPermissions` · ignores user/project/local settings files — or set `CLAUDE_CODE_RESTRICTED=1` (v2.1.248) |
...
| 🧠 **Fable 5.1 model** | `claude-fable-5-1` — the **new default Fable**, 1M context, $10/$50 per Mtok + $0.25/Mtok cache reads — most capable in the lineup · gateways without 5.1 support still resolve `fable`/`best` to Fable 5 (v2.1.257) |
| 🕐 **`timeFormat` + `timeZone` settings** | 12-hour / 24-hour / 24-hour UTC / strftime pattern for the turn-end clock and transcript-view timestamps (v2.1.257) |
| 🛡️ **Containment Escape rule in auto mode** | cloud metadata-credential fetches · egress evasion · cross-tenant reach — no longer auto-approved unless your environment marks them expected (v2.1.257) |
| 📖 **Reads outside the working dirs** | auto mode asks once before the first file read outside the working directories + `permissions.blockReadsOutsideWorkingDirectories` setting (v2.1.257) |
...
| 🪞 **`/diff` panel** | in fullscreen mode, `/diff` opens a diff panel beside the conversation showing your uncommitted changes live as Claude edits (v2.1.260) |
| 💬 **`/advisor` text form + headless `/reload-plugins`** | `/advisor <model>` / `/advisor off` work in the desktop app/Remote Control/headless sessions · `/reload-plugins` appears in the Desktop and SDK command lists (v2.1.260) |
| 🧹 **Several behavior changes** | `/cost` explains prompt-cache misses · `Ctrl+L`/`Cmd+K` clear the fullscreen transcript · permission rules with trailing text are reported invalid · `!` bash-mode always runs unsandboxed · Chrome follows the org admin setting · subagent background commands lose the 1-hour cap (v2.1.260) |
| 🩺 **`/skill-doctor`** | shows which loaded skills go unused and what each costs in context, so you can prune them (v2.1.261) |
| 📏 **Output limits + file-based subagent prompt** | `bashOutputMaxChars` + `taskOutputMaxChars` settings raise the inline output Claude receives up to 128K characters · `--append-subagent-system-prompt-file` reads the subagent system prompt from a file (v2.1.261) |
| ⌨️ **Several behavior changes** | word-editing keys always match Bash (`keybindingFlavor` no longer has any effect) · auto mode treats diagram-renderer links as uploads · `/status`/`claude doctor` gain an "Organization policy" line · `/context` estimates tokens locally when the API is unavailable (v2.1.261) |
| 📦 **`--plugin-dir` takes a folder of plugins + 1 GB tool-result cap** | point `--plugin-dir` at a parent folder and every child with a manifest loads — children added or removed while running are picked up · tool results saved to disk are capped at 1 GB, and the preview says when a saved file was truncated (v2.1.265) |
| 🗂️ **Several behavior changes** | slash commands typed mid-prompt show a match list and find plugin skills by bare name · remote MCP servers get no OAuth client registered until you sign in · `forceLoginGatewayUrl` makes a machine a gateway session from startup · VS Code archives inactive sessions (14 days by default) · `--worktree` checks out in parallel (v2.1.265) |
...
| 🌐 **Per-command `allowed_domains` + `omitClaudeMd` subagents** | in auto mode with sandboxing, Bash/PowerShell/Monitor declare the hosts a command needs and get them opened for that command alone · `omitClaudeMd` in agent frontmatter and `--agents` JSON runs a subagent without user/project/local CLAUDE.md (managed policy files still load) (v2.1.271) |
| ⚡ **Fast mode in Remote sessions + `/desktop`** | `/fast` and the host's fast-mode setting now apply in Claude Code Remote sessions (cloud and self-hosted runners) where the organization allows it · `/desktop` offers to download the Claude desktop app (v2.1.271) |
| 🚩 **New flags** | `claude self-hosted-runner --drain-marker-file <path>` reports a SIGTERM exit as a host drain · `--host-config-snapshot disk\|memory` for hosts whose config directory exceeds 64 MiB · `--accept-command <sha256>` on `claude plugin install`/`update` accepts exactly the command a prior `--json` run showed, instead of `-y` (v2.1.271) |
...
| ⏳ **A bound on the MCP startup wait + richer telemetry** | `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` caps how long the first non-interactive turn waits for MCP servers that are still connecting (`0` = don't wait) · the `claude_code.llm_request` trace span carries an `effort` attribute · a new `claude_code.managed_settings_resolved` event reports managed-settings sources and policy helper state, with redacted values and digests under `OTEL_LOG_MANAGED_SETTINGS=1` · a visible warning when memory usage is critical, with steps to free memory or restart safely (v2.1.274) |
| ⌨️ **A send-now key + claude.ai skill/plugin sync** | `Ctrl+Enter` (or `Ctrl+X Ctrl+S`) interrupts the current turn and sends every queued message at once · a session signed in with a Claude account picks up the skills and plugins enabled on claude.ai, opt out with `syncClaudeAiSkills` / `syncClaudeAiPlugins: false` · `/plugin install <plugin> --marketplace <source>` offers to add the marketplace before installing · gateway sign-in confirms the account it names before saving the credential, and `/status` shows it (v2.1.275) |
...
> 📋 Full per-version history in [`CHANGELOG.md`](./CHANGELOG.md) — manual references Claude Code `v2.1.282` (manual `v1.75.0`)
...
### 🔄 Always current
Tracks Claude Code **v2.1.282** + the Fable 5.1 / Opus 5.5 / Sonnet 5 lineup
